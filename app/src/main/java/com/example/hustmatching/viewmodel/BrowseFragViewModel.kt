...
class BrowseFragViewModel : ViewModel() {
    ...
    private var matchedPosts: ArrayList<Array<NetPost?>>? = ArrayList()
    ...
    val matchedPostsLive = MutableLiveData<List<Array<NetPost?>>> ()
        ...
        viewModelScope.launch {
            try {
                ...
                if (response.code == 0){
                    ...
                    tags.add(response.data.key1)
                    tags.add(response.data.key2)
                    tags.add(response.data.key3)
                    matchedPost.tags = tags
                    Log.d("findMatch", "解析得到匹配的发布：" + matchedPost)
                    val netPosts = arrayOfNulls<NetPost>(2)
                    netPosts[0] = myPost
